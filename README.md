...
> ### ⚠️ 你正在看的是 **beta 移植分支**（受原作者委托）
>
> - **上游仓库（本 mod 的原始设计与实现）**：<https://github.com/bwnotfound/StS2-NotEnoughDifficulty>
> - **本分支**：`beta-0.111-port` —— 把 mod 整体迁移适配到《Slay the Spire 2》**public beta v0.111.0**
>   （MegaDot/Godot 4.5.1）+ **BaseLib 3.4.7**，并在此基础上做了若干功能扩展与兼容性改造。
> - **移植**：[@Coll-ed](https://github.com/Coll-ed)，**受原作者委托**完成。
> - **著作权**：本 mod 的设计与原始实现归原作者所有；本分支只包含移植适配与扩展部分，
>   沿用上游 `LICENSE`。若要发布/分发，请以原作者的仓库为准，或先与原作者确认。
> - 移植差异与离线构建说明：[`README-移植与构建.md`](README-移植与构建.md)
> - 移植过程踩的坑与方法论（37 条）：[`杀戮尖塔2-mod写作踩坑指南.md`](杀戮尖塔2-mod写作踩坑指南.md)
> - 待办与交接：[`待办与交接-第三阶段.md`](待办与交接-第三阶段.md)

为《Slay the Spire 2》把原版 3 层塔扩展为 **5 层塔**，并提供一整套**按层精细调节的难度、地图与敌人池**配置的 mod。单人、多人联机均可用；多人下 host 端配置会自动同步给所有玩家。

> 上游版本 **1.0.1**；本移植分支适配 **beta v0.111.0 / BaseLib 3.4.7**（原上游为 BaseLib v3.3.0）。
...
## 移植说明与致谢（`beta-0.111-port` 分支新增）

### 本体（最重要的一条）

- **[bwnotfound](https://github.com/bwnotfound)** —— 本 mod（`StS2-NotEnoughDifficulty`）的**设计与全部原始实现**：
  第 4/5 层、按层难度倍率、难度预设、地图长度/密度、敌人移除列表、额外加速、ack-based 多人配置同步、
  读档兜底……思路与代码都出自他。本分支是**受他委托**做的 beta 移植，**著作权归他**；
  发布/分发请以本仓库原作者为准，或先与原作者确认。

### 兼容性互操作（移植版新增，专门致谢）

移植版很大一部分工作花在"与其它模组共存"上，感谢这些模组的作者把扩展点留得足够干净
（也让本 mod 能被**实测**验证，而不是纸上兼容）：

- **ritzukage** 的 **ActsFromThePast** 与 **RitsuLib** —— 它的 act / 精英 / BOSS 会被本 mod 的第 4/5 层抽取；
  它的自绘战斗背景（`TheBeyondBackground`）促使本移植放弃"自己造背景"，改成**改写 `parentAct`
  让它的钩子照常生效**（见踩坑指南 §3.17）。
- **YUI 系列扩展（YUI Spire / Card Expansion）**、**Act 4 Heart** 等模组 ——
  用于验证"与整幕模组共存"：本 mod 会在**第 4 幕被别的模组占用时自动顺延为第 5/6 幕**（配置里可关）。
- **皮肤 / 卡图 / 汉化类模组** —— 用于验证本地化与设置界面注入不打架。

### 本移植分支做了/没做什么

- **做了**：beta v0.111.0 适配、构建自包含（离线 feed + 自研 `tools/MakePck` 生成 `.pck`）；
  功能扩展 —— 第 4 幕精英**确定名单**（全基础精英各一次、按坐标落位、不重复）、
  第 5 幕传奇/神话连战（自研 BOSS 地图节点类 + 直线地图 + 火堆 = 场数 − 1）、
  战斗背景按"敌人自己所属的幕"切换（精英限定）、地图纹路按来源幕换色、多模组共存（顺延/互操作）。
- **没做**：没有改动上游的美术素材；没有新增第三方资源；视觉全部**程序化生成**
  （羊皮纸 / 血染 / 纹路换色 / 图标改色）。
- 移植差异与离线构建步骤见 [`README-移植与构建.md`](README-移植与构建.md)；
  37 条踩坑与方法论见 [`杀戮尖塔2-mod写作踩坑指南.md`](杀戮尖塔2-mod写作踩坑指南.md)。
