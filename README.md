...
| `7d37378` | **修复：装了 Ascension100 之后"双BOSS中间火堆没有了"** —— 那个模组为"进阶 11 地图火焰特效"挂了 `NMapScreen.SetMap` 的**纯视觉** patch（延迟一帧给地图点挂火焰），却触发了本模组 ModCompat 里"有人也改这个方法 ⇒ 整套放权"：`SetMap` postfix 首行 `return` ⇒ **合成火堆的视觉/连通注入整块被跳过**（双 boss 仍在，因为 `RunManager.GenerateMap` 前缀那条补救路径没有放权判定）。改为**按接缝区分「共存 / 放权」**：只做加法的接缝列 owner 后**继续执行**，放权日志升级为 `Warn`（每目标只报一次）且关键功能成功路径补 `Info`。同一轮**叫醒三处静默失效**：① 按层双 boss 开关与 N10 钉回第 3 层（`GenerateRooms` 放权改共存，并把"无差别清空别人的第二 boss"收窄为只清基础游戏误加在**最后一幕**的那个）；② `CombatManager.AfterCreatureAdded` 的 `Ambiguous match`（重载未给参数类型 ⇒ 整个 patch 类**每次启动都被跳过**）；③ act5 纹路金光/血光流动 + 每次爬楼换卷云样式、act4 图标黄→淡紫这三处**零调用点的孤儿方法**（着色器另加编译自检，不通过就不挂材质）。指南同步新增 §8.6（放权 vs 共存）/ §3.23（重载陷阱）/ §0.5（学会拆分） |
| `ca96d86` | **兜底拦截：没进火堆也能进第二个 BOSS**（打完双 BOSS 不会被卡住）—— 原版 `NMapScreen.RecalculateTravelability` 的双 BOSS 特判**只认"当前坐标 == 第一个 BOSS"**（命中就 `_secondBossPointNode.State = Travelable` 然后 **`return`**），于是有两个洞：① 跳过火堆直连时，一旦地图/连线/顺序有偏差（别的 mod 改地图、注入失败、读档重建）第二个 BOSS 就停在 `Untravelable` ⇒ 进不去；② **进了火堆再出来**时"最后一个已访问坐标"是火堆的**网格外虚拟坐标**，特判不成立，只能落到为网格内节点设计的 `MapTravel.GetTravelablePointsFrom(火堆点)`，不保证有结果 ⇒ 同样点不到。改为**三层兜底（只加不删）**：`IsBetweenBosses` 判定"序列进行中"（当前坐标 = 第一个 BOSS 或任一合成节点）→ 每次可通行性重算后 / 每次地图 `Open` 后把第二个 BOSS 显式置为 `Travelable` → 再在 `NMapPoint.OnRelease` 前缀里做**点击瞬间的硬拦截**（该方法 IL 第一行就是 `if (!IsTravelable) return;`） |
| `f8bce3f` | **修复：第四层双 BOSS 的火堆"进入即卡死"** + **难度强化被 /100 吃掉（血量几乎没涨）** —— ① `RunManager.EnterMapPointInternal` 会**把 actFloor 写进 run state**（IL 实证：`IL_006F ldfld actFloor → IL_0074 callvirt RunState::set_ActFloor`），而合成房间拿到的是**网格外虚拟行** `coord.row + 1`（实测 64~68）⇒ 进一次火堆 `ActFloor` 就被污染（它同时是难度公式与其它按层逻辑的输入），改为与原版同口径的"第二 BOSS 节点行号 + 1"；② `SpeedMultiplierController` 旧实现**每帧无条件**抢 `Engine.TimeScale`，与做 hit-stop 的 mod（实测 HitStop，日志里成片 `[HitStop] 检测到 TimeScale 被修改为 1.4，强制恢复为 0`）拉锯 —— 一旦 0 留在引擎里，`Tween`/`SceneTreeTimer`/`AnimationPlayer`（进房演出全靠它们）全部停摆 ⇒ 画面停住 = "卡死"，改为"**别人定帧时一律不抢** + 定帧持续超 3 秒才兜底拉回"；③ 难度公式与配置文案**差 100 倍**（文案：第 10 层 → 10×0.1×Y = **+100%**；代码 `1 + ActFloor×0.1×Y/100` ⇒ 只有 **+1%**，Y=1 时肉眼无感），去掉 /100 并 clamp 到 100 倍防呆 |
| `e3ce085` | **修复：第五幕（本模组最终幕）重新出现已经打过的 BOSS** —— 取证（同一局，跨日志）：该局 `Final act list (6): EXORDIUM#1 → HIVE#2 → **GLORY#3** → THE_ENDING#4 → 本模组ACT4#5 → 本模组ACT5#6`；第 3 幕 GLORY 的 BOSS = `DOORMAKER_BOSS`（已击败），而第 6 幕的最终 BOSS **又是 DOORMAKER_BOSS**。根因："考验"档的 `_finale = Act3FinalBoss(state)` = **第 3 层当前分配的那个 boss**（玩家在第 3 幕已亲手打过）⇒ 必然重复；极限档早就是"从没打过的第 3 层池里抽"，考验档漏了这条判据。修法：① 考验档最终 BOSS 改为**优先从"第 3 层没打过"的池里抽**，全部打过才退回 `Act3FinalBoss`；② `Unvisited` 的兜底不再"回退成该层全池随机"（那等于把打过的 boss 再抽一遍），改为**跨层"全局没打过"**兜底；③ 新增"本幕已选"集合（整幕模组会把同一个 boss 注册进多层池，只按层去重不够）；④ 极限档编排循环加**硬步数上限**（旧写法靠 `anyLeft` 的 break 收工，一旦内部出现"跳过已选过的" continue 分支就可能永远转下去 ⇒ 卡死主线程） |
| `5c6e283` | **修复：第二个 BOSS 打的其实是第一个 BOSS**（双 BOSS 第二场重复）—— 日志实证（同一局、同一会话、无读档）：act4 先打赢 `KNOWLEDGE_DEMON_BOSS` → 进合成火堆（坐标 (53,63)）→ 点第二个 BOSS 节点 (3,13) → `Creating NCombatRoom … encounter=KNOWLEDGE_DEMON_BOSS` **还是同一个**。根因：原版 `CreateRoom` 的取怪是 `model as EncounterModel ?? Act.PullNextEncounter(roomType)`，而 `PullNextEncounter(Boss)` 返回 `RoomSet.NextBossEncounter`（**整幕固定那一个** = 第一个 BOSS）；原版自己的双 BOSS 之所以正确，是因为它走"打完第一个 BOSS → 奖励界面继续"那条路，**把 `SecondBossEncounter` 显式当 model 传进 CreateRoom**，而本模组在两个 BOSS 之间插了火堆 ⇒ 玩家是**点地图节点**过去的。修法：① 新增 `SecondBossEncounterPatch`（`ActModel.PullNextEncounter` 前缀：正要进入的坐标 == 本层第二 BOSS 坐标 ⇒ 直接返回该层的 `SecondBossEncounter`，**只改内容、不碰流程**）；② `RunManager.EnterMapCoord` 前缀登记"正在进入的坐标"；③ 建图前加**不变量兜底**：发现"第二 boss == 首个 boss"就自动从该层池换一个（并 Warn） |
